...
//		toolbar_size_ = toolbar_.IsHorizontal() ? size.cy : size.cx;
	...
	rebar_wnd_.AddBand(&filter_bar_wnd_, CSize(300, rect.Height()), _T("Fi&nd")/*underscore ? _T("Fi&nd") : _T("Find")*/, BAND_FILTER, 100);

	addr_box_wnd_.GetWindowRect(rect);
	rebar_wnd_.AddBand(&addr_box_wnd_, CSize(300, rect.Height()), _T("A&ddress")/*underscore ? _T("A&ddress") : _T("Address")*/, BAND_ADDRESS, 150);
...
//	PaneLayoutInfo test(folders,		CRect( 0,  0,  20,  60), _T("Folders"),		PANE_NORMAL, -1, EDGE_NONE, EDGE_LEFT);
	...
	static PaneLayoutInfo layout[]=
	{
		...
		PaneLayoutInfo(preview,   CRect(20,  0,  80,  35), L"Preview",   PANE_HIDDEN, -1, EDGE_NONE, EDGE_NONE),
		PaneLayoutInfo(mainview,  CRect(20, 35,  80, 100), L"Images",    PANE_NORMAL | PANE_NO_CLOSE, -1, EDGE_NONE, EDGE_NONE),
		PaneLayoutInfo(infobar,   CRect(80,  0, 100, 100), L"Info ",     PANE_HIDDEN, -1, EDGE_NONE, EDGE_RIGHT, 300 / 8),
	};
