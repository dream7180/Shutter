...
void ExifView::ViewPhoto(PhotoInfoPtr photo, ViewerType type)
{
	...
	if (!viewer_link_.IsValid())
	{
		...
		viewer->Create(_T("Shutter Image Viewer"));
	}
	else
	{
...
void ExifView::DeletePhotos(bool warn)
{
	try
	{
		...
		{
			if (warn)
				new BalloonMsg(&GetListCtrl(), _T("There Are No Images Selected"),
					_T("Please select images to delete."), BalloonMsg::IERROR);
			return;
		}
		...
		for (size_t i= 0; i < count; ++i)
			...
			{
				if (warn)
					new BalloonMsg(&GetListCtrl(), _T("Cannot Delete images"),
					_T("Selected images cannot be deleted.."), BalloonMsg::IERROR);
				return;
			}
...
void ExifView::FileOperation(bool copy)
{
	try
	{
		...
		if (selected.empty())
		{
			new BalloonMsg(&GetListCtrl(), _T("There Are No Selected Images"),
				_T("Please select images to copy/move first."), BalloonMsg::IERROR);
			return;
		}
...
void ExifView::OnTaskResize()
{
	try
	{
		...
		{
			new BalloonMsg(&GetListCtrl(), _T("There Are No Selected Images"),
				_T("Please select images to resize first."), BalloonMsg::IERROR);
			return;
		}
...
void ExifView::OnTaskGenSlideShow()
{
	try
	{
		...
		{
			new BalloonMsg(&GetListCtrl(), _T("There Are No Selected images"),
				_T("Please select images to generate slide show first."), BalloonMsg::IERROR);
			return;
		}
...
void ExifView::OnTaskGenHTMLAlbum()
{
	try
	{
		...
		{
			new BalloonMsg(&GetListCtrl(), _T("There Are No Selected images"),
				_T("Please select images to generate HTML album first."), BalloonMsg::IERROR);
			return;
		}
...
void ExifView::OnTaskCopyTagged()
{
	try
	{
		...
		{
			new BalloonMsg(&GetListCtrl(), _T("There Are No Tagged Images"),
