...
std::vector<COLORREF> GetPhotoCtrlDefaultColors(const ApplicationColors& app_colors)
{
	...
	colors[PhotoCtrl::C_BACKGND] = background; // ::GetSysColor(COLOR_WINDOW);
...
//	colors[PhotoCtrl::C_FRAME] = ::GetSysColor(COLOR_3DFACE);
	colors[PhotoCtrl::C_SEPARATOR] = app_colors[AppColors::SecondarySeparator];// CalcNewColor(app_colors[AppColors::Separator], background, 0.5f);// ::GetSysColor(COLOR_HIGHLIGHT);
	colors[PhotoCtrl::C_DIM_TEXT] = app_colors[AppColors::DimText];// CalcNewColor(text, 50.0f);
	colors[PhotoCtrl::C_EDIT_BACKGND] = app_colors[AppColors::EditBox];// ::GetSysColor(COLOR_WINDOW);
